...
class HistorySearch extends Component {
  ...
  historyDelete = id => {
    ...
    const filterData = historyList.filter(historyItem => historyItem.id !== id)
    this.setState({historyList: filterData})
  }

  render() {
    const {historyList, searchInput} = this.state
    const filterSearch = historyList.filter(eachItem =>
      eachItem.title.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return (
      <div className="history-search-main-container">
        <div className="navbar">
          <div className="history-search-top-container">
            <img
          ...
          <div className="search-history-list-main-container">
            {filterSearch.length > 0 ? (
              <ul className="search-history-list-container">
                {filterSearch.map(eachHistory => (
                  <HistoryItem
                    eachHistory={eachHistory}
                    historyDelete={this.historyDelete}
                    key={eachHistory.logoUrl}
                  />
                ))}
              </ul>
            ) : (
